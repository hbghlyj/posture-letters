...
- **K:** rebuilt as a kneeling figure. Torso, neck, head and the kneeling near leg stack into one plumb line with the knee planted directly under the torso, giving the letter a dead-vertical left side. The far leg extends well out to the side at a bent knee, so the lower diagonal gets the extra length a K needs to look balanced against the short lower stem. The near arm raises straight up and out at about forty-five degrees as the upper diagonal, ending in an open hand serif, and the far arm is pinned flat along the hip and thigh so the centre stays solid with no trapped white between the two diagonals.
- **L:** rebuilt as a figure lying flat on its back with the hips flexed to a right angle. Because the legs plus lower torso are naturally longer than the upper torso, lifting them completely straight up makes the tall vertical stem, with the knees locked for rigidity and the toes pointing up as a terminal peak. The upper back, shoulders and head lie flat along the ground running right to form the shorter horizontal base, and the arms rest on the floor with the hands near the head, adding weight to the end of the bar. The two raised legs are set apart with a real gap between them and each carries an engraved contour down thigh and shin; the arms are likewise held clear of the torso edge with a deltoid wedge at the shoulder and a crease along each forearm, so every limb reads separately instead of fusing into a slab.
