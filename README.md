...
- **Spacing (all glyphs):** the font is now proportionally spaced. A single fixed advance suited the wide poses but stranded narrow figures like I and J in a sea of white, so each glyph's advance is fitted to its own ink with a constant 55-unit sidebearing, capped at the em width for poses that already fill it.
...
- **L:** rebuilt as an upright seated posture. The spine is straight and strictly vertical, perpendicular to the ground, with the head facing forward in line with it. Torso, arms and legs are sized on the font's own baseline ratios — arm 1.22x torso, leg 1.84x torso — so the seated figure has the same build as the standing ones rather than a long trunk with stub arms. Both legs extend fully straight ahead along the floor to form the horizontal baseline, feet together and flexed square so the toes point up; they reach far enough right that the arm is proportional to a normal cap L, and they root inside the torso's own width so pelvis and thighs fuse into one silhouette with no notch at the corner. Instead of resting at the hips, the arms are raised completely overhead parallel to the spine, spreading slightly apart, ending in wide-open palms with the fingers pointing at the ceiling.
