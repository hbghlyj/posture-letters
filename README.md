...
- **C:** the overhead arms now taper toward the wrist and finish in modelled hands with reaching fingers, as G's do. Previously each arm ended in a plain round terminal that came out wider than the forearm itself and read as a blob beside the tapered shins.
...
- **E:** the two arm crossbars now run dead horizontal and end in open hands, matching F's construction — previously the top bar rose and the middle bar fell away, so the prongs read as drooping limbs rather than deliberate bars. The two baseline legs were also brought close enough to overlap into a single stroke: held apart they read as a fourth horizontal bar and the E appeared to have four prongs.
- **G:** rebuilt as one continuous body with a deliberate twist at the waist. The arms stretch overhead and curve forward to make the upper crest, the head looking down and the chest turned inward so the top reads as a rounded, protected profile. The middle torso then twists — marked by an engraved seam — reorienting the body so the lower torso faces outward to the left, which puts the hips where the knees can bend up and forward and sweep the thighs through the wide bottom arc. The shins push up the right side and the feet angle sharply back inward, running horizontally parallel to each other into the counter as G's terminal crossbar, kept to a believable foot length of roughly half the shin.
...
- **N:** rebuilt as a dynamic back-bend. The straight arms are planted on the ground and stand vertically to form the left stroke, spaced as on M so both weight-bearing arm columns share the same stance, with flat supporting hands pressed on the floor; the torso and thighs slope down-right from the shoulders to the knees as the diagonal; and the knees rest on the ground so both lower legs rise straight up into the air as the right stroke. The hands lie flat along the floor as on M, wrists bent and fingers running out horizontally to give the base of the left stroke a serif, and the shin is kept shorter than the whole arm so the right stroke does not out-reach the weight-bearing left one. The head hangs back past the planted shoulders, its face turned left as on L.
